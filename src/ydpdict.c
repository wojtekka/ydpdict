...
int main(int argc, char **argv)
{
  ...
  do {
	...
	break;
      case KEY_F(1):
        def = strdup(_("\
{\\b ydpdict-" VERSION "\\line\\cf1(c) 1998-2002 by wojtek kaniewski}\
\\par\\pard{\
...
kontakt z autorem: {\\b wojtekka@irc.pl}\
	...
	break;
	  ...
	  if (defline > 0) defline--;
          ...
          if (menu > 0) menu--; else if (pos > 0) pos--;
	  defline = 0;
	  defupd = 1;
	}
	break;
...
void preparewins() {
  ...
  for (x = 0; x < ysize; x++) switch(charset) {
    case 0:
...
int showdef(char *def, int first) {
  ...
  while(*def) {
	...
	if (*def == ' ') def++;
	if (!lastnl) {
  	  if (!strcmp(token, "par") || !strcmp(token, "line")) newline_ = 1;
	  margin = 0;
	}
    ...
    if (dispword && lp) {
      if (50 - xpos < lp) {
        wprintw(defwin, is_visible("\n"));
	ypos++;
	if (margin) wprintw(defwin, is_visible("   "));
